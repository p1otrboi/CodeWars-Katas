...
{
    internal class Does_my_number_look_big_in_this
    {
        public static bool Narcissistic(int value)
        {
            int[] numarray = value.ToString().Select(c => (int)Char.GetNumericValue(c)).ToArray();
            List<double> list = new List<double>();
            foreach (double number in numarray)
                list.Add(Math.Pow(number, numarray.Length));
            if (list.Sum() == value)
                return true;
            else
                return false;
